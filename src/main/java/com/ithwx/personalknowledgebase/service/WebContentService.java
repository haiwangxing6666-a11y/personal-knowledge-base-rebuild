...
public class WebContentService {
    ...
    public WebPage fetch(String url) {
        ...
        try {
            ...
            try (InputStream body = response.body()) {
                ...
                return parseContent(uri, contentType, bytes);
            }
    ...
    WebPage parseContent(URI uri, String contentType, byte[] bytes) {
        if (contentType.startsWith("text/plain")) {
            return createPage(uri, uri.getHost(), new String(bytes, StandardCharsets.UTF_8));
        }
        ...
        return createPage(uri, title, content == null ? "" : content.text());
    }

    private WebPage createPage(URI uri, String title, String text) {
        if (text.isBlank()) {
            throw new IllegalArgumentException("网页中没有可提取的正文");
        }
        return new WebPage(uri.toString(), title.strip(), text.strip());
    }
